...
Este código representa una página de inscripción de un usuario a un campeonato deportivo. La página tiene dos pestañas: "Ficha de Inscripción" y "Confirmación de Inscripción", la cual utiliza condicionales v-if y v-else para alternar entre ellas.
...
El siguiente bloque permite seleccionar entre las pestañas "Ficha de Inscripción" y "Confirmación de Inscripción".  
...
Muestra un formulario con campos para seleccionar un campeonato y marcar las pruebas en las que el participante desea inscribirse, manejando eventos como el cambio de selección de campeonato (@change="listarPruebasDelCampeonato") y el envío del formulario (@submit.prevent="inscribirse()"). Además,
permite la carga de archivos (comprobante de pago y ficha de inscripción).
...
Muestra una tabla con información sobre los campeonatos en los que el usuario está inscrito, proporcionando enlaces para descargar comprobantes de pago y fichas de inscripción.
Incluye la subida de archivos para confirmar el pago y la ficha de inscripción.
...
```
- mounted(): Método que se ejecuta después de que el componente ha sido montado.
...
```
- methods: Contiene los métodos (funciones) utilizados en el componente.
